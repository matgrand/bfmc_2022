...
class Detection:
    ...
    #init 
    def __init__(self, trafficlight=False, sign=False, obstacle=False, show_ROI=False) -> None:
        #traffic light classifier #trafficlight is abbreviated in tl

        if show_ROI:
            cv.namedWindow('trafficlight_roi', cv.WINDOW_NORMAL) if trafficlight else None
            cv.namedWindow('red_stripe', cv.WINDOW_NORMAL) if trafficlight else None
            cv.namedWindow('yellow_stripe', cv.WINDOW_NORMAL) if trafficlight else None
            cv.namedWindow('green_stripe', cv.WINDOW_NORMAL) if trafficlight else None
            cv.namedWindow('signs_roi', cv.WINDOW_NORMAL) if sign else None
            cv.namedWindow('obstacles_roi', cv.WINDOW_NORMAL) if obstacle else None
    ...
    def classify_traffic_light(self, frame, conf_threshold=0.3):
        SIZE = (32, 32)
        ROI = [30,260,-100,640]
        ...
        conf = soft_preds[trafficlight_index]
        if conf > conf_threshold:
            ...
            if predicted_trafficlight != self.tl_names[-1]:     
                ...
                return predicted_trafficlight, conf, tl_color
        return None, 0.0, None
                

    def classify_sign(self, frame, conf_threshold=0.8):
        """
        Sign classifiier:
    ...
    def find_trafficlight_color(self, trafficlight_roi, max_deviation=0.5):
        """
        ...
        #show stripes
        if self.show_ROI:
            cv.imshow('red_stripe', red_stripe)
            cv.imshow('yellow_stripe', yellow_stripe)
            cv.imshow('green_stripe', green_stripe)
            cv.waitKey(1)
        ...
        brightest_color_value = avg_array[brightest_color_index]
