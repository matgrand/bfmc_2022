...
class PathPlanning(): 
    ...
    def augment_path(self, default_distance=150, tol=0.01): #100
        ...
        for i in range(len(self.path)-1):
            ...
            for n in self.intersection_in:
                ...
                if (nx-curr_x)**2 + (ny-curr_y)**2 <= tol**2:
                    ...
                    self.stop_points_on_path.append(np.array([nx, ny]))
        ...
        #END OF WHILE
        print('END OF WHILE')
        print(f'len of path_data: {len(self.path_data)}')
        print(f'len of path: {len(self.path)}')
        curr_stop_idx = None
        for i in reversed(range(len(self.path_data))):
            p = self.path[i]
            diff = np.linalg.norm(p-self.stop_points_on_path, axis=1)
            if np.min(diff) < 1e-5:
                curr_stop_idx = i
            dist = (curr_stop_idx - i)*0.01 if curr_stop_idx is not None else 1.0
            self.path_data[i][3] = dist
    ...
    def draw_path(self):
        ...
        for node in self.list_of_nodes:
            ...
            p = mR2pix(p)
            cv.circle(self.map, p, 5, (0, 0, 255), -1)
            #add node number
            cv.putText(self.map, str(node), p, cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        ...
        for edge in self.list_of_edges:
            ...
            p1 = mR2pix(p1)
            p2 = mR2pix(p2)
            #cv.line(self.map, p1, p2, (0, 255, 255), 2)
        ...
        for point in self.route_list:
            ...
            p = np.array([x,y], dtype=np.float32)
            p = mR2pix(p)
            cv.circle(self.map, p, 5, (255, 0, 0), 1)
